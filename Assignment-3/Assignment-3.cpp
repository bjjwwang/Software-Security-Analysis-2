...
void AbstractExecution::bufOverflowDetection(const SVF::SVFStmt* stmt) {
	if (!SVFUtil::isa<CallICFGNode>(stmt->getICFGNode())) {
		if (const GepStmt* gep = SVFUtil::dyn_cast<GepStmt>(stmt)) {
			// TODO: 1 update GepObj Offset From Base

			// TODO: 2 compare access offset from base Obj with the size of base Obj

			// TODO: 3 call reportBufOverflow if buf overflow is detected
		}
	}
}
